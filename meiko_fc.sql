...
CREATE TABLE `t_product` (
  ...
  `flg_market` varchar(1) DEFAULT '1' COMMENT '1:塾用教材 2:市販教材',
...
CREATE TABLE `t_classroom` (
  ...
  `en_code1` varchar(1) DEFAULT NULL COMMENT 'ENコード（掛） 1:有 2:無',
  `en_code2` varchar(1) DEFAULT NULL COMMENT 'ENコード（ｸﾚｼﾞｯﾄ） 1:有 2:無',
...
CREATE TABLE `t_order` (
  ...
  `payment_method` varchar(1) NOT NULL COMMENT '支払方法 1:買掛 2:クレジットカード 3:代金引換',
...
CREATE TABLE `t_owner` (
  ...
  `email` varchar(256) DEFAULT NULL COMMENT 'メールアドレス',
  `payment_method1` varchar(1) DEFAULT '0' COMMENT '買掛 0:利用しない 1:利用する',
  `payment_method2` varchar(1) DEFAULT '0' COMMENT 'クレジットカード 0:利用しない 1:利用する',
  `payment_method3` varchar(1) DEFAULT '0' COMMENT '代金引換 0:利用しない 1:利用する',
  `flg_complete` varchar(1) DEFAULT '1' COMMENT '登録フラグ 1:未 2:完了',
  `regist_time` datetime NOT NULL COMMENT '登録日',
  `update_time` datetime NOT NULL COMMENT '更新日',
  `status` varchar(1) DEFAULT '0' COMMENT '状態 0:通常 9:削除済',
